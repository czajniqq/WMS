...
function initCharts(cpuCanvasId, ramCanvasId, netCanvasId) {
  ...
  // Opcje dla Sieci (bez limitu y=100)
  ...
  netOptions.plugins.legend.display = true; // Pokazujemy legendę dla Wysyłania/Odbierania
...
function updateCharts(metricsData) {
  ...
  // Obliczanie transferu KB/s (Delta pomiędzy pomiarami)
  const sentData = [];
  const recvData = [];
  for (let i = 0; i < sorted.length; i++) {
    if (i === 0) {
      ...
      if (timeDiffSec > 0 && curr.uptime_seconds >= prev.uptime_seconds) {
        ...
        sentData.push(0); recvData.push(0); // np. restart komputera
      }
    }
  }
