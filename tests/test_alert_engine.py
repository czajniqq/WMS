...
def test_cpu_alert_generated(db_session):
    agent = Agent(hostname="cpu-test", ip_address="1.2.3.4", agent_version="1.0", registered_at=datetime.now(timezone.utc), last_seen=datetime.now(timezone.utc))
...
def test_ram_alert_generated(db_session):
    agent = Agent(hostname="ram-test", ip_address="1.2.3.5", agent_version="1.0", registered_at=datetime.now(timezone.utc), last_seen=datetime.now(timezone.utc))
    db_session.add(agent)
    db_session.commit()
...
def test_no_duplicate_alert(db_session):
    agent = Agent(hostname="dup-test", ip_address="1.2.3.6", agent_version="1.0", registered_at=datetime.now(timezone.utc), last_seen=datetime.now(timezone.utc))
    db_session.add(agent)
    db_session.commit()
    evaluate(db_session, agent.id, cpu_percent=95.0, ram_percent=10.0)
    evaluate(db_session, agent.id, cpu_percent=95.0, ram_percent=10.0)
